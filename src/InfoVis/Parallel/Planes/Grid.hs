...
updateGrids :: RealFloat a => GridsAction -> Vector3 a -> Grids -> IO Grids
...
findSelection :: RealFloat a => Configuration -> Vector3 a -> Maybe Cell
...
spacingAndSize :: RealFloat a => Configuration -> (a, a)
...
makeGrid configuration@Configuration{..} =
  let
    (spacing, size) = spacingAndSize configuration :: (Resolution, Resolution)
    grid =
      concat [
        concat $
        [
...
planesGeometry :: Configuration -> Cells -> [Vertex3 Resolution]
...
linesGeometry :: Configuration -> [Double] -> [Vertex3 Resolution]
linesGeometry configuration@Configuration{..} points =
  let
    spacing = fst $ spacingAndSize configuration
